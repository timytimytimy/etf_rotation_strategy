...
def run_backtest_full(code_list, code_names, code_category, 
    ...
    print("  1. 凯利公式仓位管理")
    print("  2. 动态止损（-5%固定）")
    print("  3. 多因子排序（动量+波动率+夏普）")
    print("  4. 相关性过滤（相关性<0.7）")
    print()
    ...
    for code in code_list:
        ...
        if len(wins) > 5 and len(losses) > 5:
            ...
            kelly = max(0, min(kelly * 0.5, 0.5))  # 半凯利，最高50%
    ...
    for i in range(N+1, len(data)):
        ...
        for code in code_list:
            ...
            # 综合得分
            scores[code] = 0.5 * momentum + 0.25 * vol_score + 0.25 * sharpe * 10
                    ...
                    if total_kelly > 0:
                        for code in selected:
                            if code in etf_stats:
                                ...
                                holdings[code] = min(w, 0.5)  # 最高50%
                    else:
                        # 如果凯利都是0，等权分配
                        for code in selected:
                            holdings[code] = 1.0 / len(selected)
...
if __name__ == '__main__':
    ...
    result['nav_series'].to_csv('backtest_full_v2.csv')
    print()
    print("✅ 净值曲线已保存")
