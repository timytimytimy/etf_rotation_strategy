# 进阶稳健版ETF轮动策略
# 特点：择时模块 + 波动率过滤 + 交易成本 + 双持仓 + 止损

import akshare as ak
import pandas as pd
import numpy as np
from sklearn.linear_model import LinearRegression
import time
from datetime import datetime
import json
import os

# 候选池（增加货币基金ETF作为避险）
code_list = ['510880', '159915', '513100', '518880', '511880']
code_names = {
    '510880': '红利ETF',
    '159915': '创业板ETF',
    '513100': '纳指ETF',
    '518880': '黄金ETF',
    '511880': '银华日利(货币)'
}

# 进阶参数
CASH_RATIO = 0.20
POSITION_RATIO = 0.40
SWITCH_THRESHOLD = 0.20
STOP_LOSS = 0.08
TRADING_COST = 0.001  # 交易成本0.1%
VOL_THRESHOLD = 0.03  # 波动率阈值3%

# 择时参数
TIMING_CODE = '510300'  # 沪深300作为择时基准
MA_SHORT = 20
MA_LONG = 60

TRACK_FILE = 'track_record_advanced.json'

def calculate_score(srs, N=25):
    if srs.shape[0] < N:
        return np.nan
    x = np.arange(1, N+1)
    y = srs.values / srs.values[0]
    lr = LinearRegression().fit(x.reshape(-1, 1), y)
    slope = lr.coef_[0]
    r_squared = lr.score(x.reshape(-1, 1), y)
    return 10000 * slope * r_squared

def calculate_volatility(srs, N=5):
    """计算N日年化波动率"""
    if len(srs) < N:
        return 0
    rets = srs.pct_change().dropna().tail(N)
    return rets.std() * np.sqrt(252)

def get_market_timing():
    """获取择时信号"""
    try:
        end_date = datetime.now().strftime('%Y%m%d')
        start_date = (datetime.now() - pd.Timedelta(days=120)).strftime('%Y%m%d')
        
        df = ak.fund_etf_hist_em(symbol=TIMING_CODE, period='daily',
            start_date=start_date, end_date=end_date, adjust='hfq')
        df.index = pd.to_datetime(df['日期'])
        df = df.sort_index()
        
        close = df['收盘']
        ma_short = close.rolling(MA_SHORT).mean().iloc[-1]
        ma_long = close.rolling(MA_LONG).mean().iloc[-1]
        current = close.iloc[-1]
        
        if current < ma_long:
            return 'BEAR', '大盘跌破60日均线，清仓观望'
        elif current < ma_short:
            return 'CAUTION', '大盘跌破20日均线，减仓50%'
        else:
            return 'BULL', '大盘处于多头趋势'
    except Exception as e:
        return 'UNKNOWN', f'择时数据获取失败: {e}'

def load_track():
    if os.path.exists(TRACK_FILE):
        with open(TRACK_FILE, 'r') as f:
            return json.load(f)
    return {
        'start_date': None,
        'holdings': [],
        'signals': [],
        'simulated_nav': 1.0,
        'daily_returns': [],
        'cash': CASH_RATIO,
        'total_cost': 0.0,
        'timing_state': 'UNKNOWN'
    }

def save_track(track):
    with open(TRACK_FILE, 'w') as f:
        json.dump(track, f, ensure_ascii=False, indent=2)

def get_latest_signal():
    end_date = datetime.now().strftime('%Y%m%d')
    start_date = (datetime.now() - pd.Timedelta(days=60)).strftime('%Y%m%d')

    df_list = []
    for code in code_list:
        try:
            df = ak.fund_etf_hist_em(symbol=code, period='daily',
                start_date=start_date, end_date=end_date, adjust='hfq')
            df.insert(0, 'code', code)
            df_list.append(df)
            time.sleep(1)
        except Exception as e:
            print(f"获取{code}失败: {e}")
            continue

    if not df_list:
        return None

    all_df = pd.concat(df_list, ignore_index=True)
    data = all_df.pivot(index='日期', columns='code', values='收盘')
    available_codes = [c for c in code_list if c in data.columns]
    data = data[available_codes]
    data.index = pd.to_datetime(data.index)
    data = data.sort_index()

    returns = {}
    prices = {}
    volatilities = {}
    for code in available_codes:
        prices[code] = float(data[code].iloc[-1])
        if len(data) > 1:
            returns[code] = (data[code].iloc[-1] / data[code].iloc[-2] - 1) * 100
        else:
            returns[code] = 0
        volatilities[code] = calculate_volatility(data[code])

    N = 25
    scores = {}
    for code in available_codes:
        recent = data[code].tail(N)
        if len(recent) == N:
            scores[code] = calculate_score(recent, N)

    return {
        'date': data.index[-1].strftime('%Y-%m-%d'),
        'available_codes': available_codes,
        'scores': scores,
        'prices': prices,
        'returns': returns,
        'volatilities': volatilities
    }

def decide_holdings_advanced(scores, current_holdings, prices, volatilities, timing):
    """进阶版持仓决策"""
    if not scores:
        return current_holdings, [], 0

    # 择时判断
    timing_state, timing_msg = timing
    
    # 清仓信号
    if timing_state == 'BEAR':
        new_holdings = [{
            'code': '511880',  # 货币基金
            'buy_price': prices.get('511880', 1),
            'buy_date': datetime.now().strftime('%Y-%m-%d'),
            'position': 0.8
        }]
        return new_holdings, [{'action': '择时清仓', 'reason': timing_msg}], TRADING_COST
    
    # 减仓信号
    position_multiplier = 0.5 if timing_state == 'CAUTION' else 1.0

    # 过滤高波动ETF
    filtered_scores = {k: v for k, v in scores.items() 
                       if volatilities.get(k, 0) < VOL_THRESHOLD or k == '511880'}
    
    if not filtered_scores:
        filtered_scores = scores
    
    sorted_etfs = sorted(filtered_scores.items(), key=lambda x: -x[1])
    top2_codes = [sorted_etfs[0][0], sorted_etfs[1][0]] if len(sorted_etfs) >= 2 else [sorted_etfs[0][0]]

    changes = []
    total_cost = 0

    if not current_holdings:
        new_holdings = []
        for code in top2_codes:
            pos = POSITION_RATIO * position_multiplier
            new_holdings.append({
                'code': code,
                'buy_price': prices.get(code, 1),
                'buy_date': datetime.now().strftime('%Y-%m-%d'),
                'position': pos
            })
        changes.append({'action': '建仓', 'codes': top2_codes, 'timing': timing_msg})
        total_cost = TRADING_COST * len(top2_codes)
        return new_holdings, changes, total_cost

    # 检查止损
    final_holdings = []
    for h in current_holdings:
        code = h['code']
        if code in prices:
            loss = (prices[code] - h['buy_price']) / h['buy_price']
            if loss < -STOP_LOSS:
                changes.append({'action': '止损', 'code': code, 'loss': f'{loss*100:.2f}%'})
                total_cost += TRADING_COST
                continue
        final_holdings.append(h)

    # 调整仓位
    for h in final_holdings:
        h['position'] = POSITION_RATIO * position_multiplier

    # 换仓逻辑
    current_codes = [h['code'] for h in final_holdings]
    
    for new_code, new_score in sorted_etfs[:2]:
        if new_code not in current_codes and len(final_holdings) < 2:
            if len(final_holdings) < 2:
                final_holdings.append({
                    'code': new_code,
                    'buy_price': prices.get(new_code, 1),
                    'buy_date': datetime.now().strftime('%Y-%m-%d'),
                    'position': POSITION_RATIO * position_multiplier
                })
                changes.append({'action': '加仓', 'code': new_code})
                total_cost += TRADING_COST
        elif new_code not in current_codes:
            for j, h in enumerate(final_holdings):
                old_code = h['code']
                old_score = filtered_scores.get(old_code, 0)
                if new_score > old_score * (1 + SWITCH_THRESHOLD):
                    changes.append({
                        'action': '换仓',
                        'from': old_code,
                        'to': new_code,
                        'reason': f'得分提升{(new_score/old_score-1)*100:.1f}%'
                    })
                    final_holdings[j] = {
                        'code': new_code,
                        'buy_price': prices.get(new_code, 1),
                        'buy_date': datetime.now().strftime('%Y-%m-%d'),
                        'position': POSITION_RATIO * position_multiplier
                    }
                    total_cost += TRADING_COST * 2
                    break

    return final_holdings, changes, total_cost

def format_message(result, track, changes, timing):
    scores = result['scores']
    prices = result['prices']
    returns = result['returns']
    volatilities = result['volatilities']
    holding_codes = [h['code'] for h in track['holdings']] if track['holdings'] else []
    timing_state, timing_msg = timing

    msg = f"🚀 **进阶稳健版ETF轮动信号** [{result['date']}]\n\n"

    # 择时状态
    timing_emoji = {'BULL': '🟢', 'CAUTION': '🟡', 'BEAR': '🔴', 'UNKNOWN': '⚪'}
    msg += f"**择时信号：**{timing_emoji.get(timing_state, '⚪')} {timing_msg}\n\n"

    # 今日持仓
    if holding_codes:
        msg += f"**今日持仓：**\n"
        for h in track['holdings']:
            code = h['code']
            pos = h.get('position', POSITION_RATIO)
            vol = volatilities.get(code, 0)
            vol_warning = " ⚠️波动高" if vol > VOL_THRESHOLD else ""
            msg += f"- {code_names.get(code, code)}（{pos*100:.0f}%仓位）{vol_warning}\n"
        msg += f"- 现金（{track['cash']*100:.0f}%仓位）\n"

    # 调仓记录
    if changes:
        msg += f"\n**今日调仓：**\n"
        for c in changes:
            if c['action'] == '择时清仓':
                msg += f"- 🛑 {c['action']}：{c['reason']}\n"
            elif c['action'] == '建仓':
                msg += f"- 📌 {c['action']}：{', '.join([code_names.get(x, x) for x in c['codes']])}\n"
                if 'timing' in c:
                    msg += f"  ({c['timing']})\n"
            elif c['action'] == '止损':
                msg += f"- 🛑 {c['action']}：{code_names.get(c['code'], c['code'])}（{c['loss']}）\n"
            elif c['action'] == '换仓':
                msg += f"- 🔄 {c['action']}：{code_names.get(c['from'], c['from'])} → {code_names.get(c['to'], c['to'])}（{c['reason']}）\n"

    # 得分排名
    msg += f"\n**各ETF得分排名：**\n"
    msg += "| 排名 | ETF | 得分 | 涨跌 | 波动率 | 状态 |\n"
    msg += "|------|-----|------|------|--------|------|\n"
    sorted_codes = sorted(scores.items(), key=lambda x: -x[1])
    for rank, (code, score) in enumerate(sorted_codes, 1):
        ret = returns.get(code, 0)
        ret_str = f"+{ret:.2f}%" if ret >= 0 else f"{ret:.2f}%"
        vol = volatilities.get(code, 0)
        vol_str = f"{vol*100:.1f}%"
        vol_mark = "🔥" if vol > VOL_THRESHOLD else ""
        status = "✅持有" if code in holding_codes else ""
        msg += f"| {rank} | {code_names.get(code, code)} | {score:.1f} | {ret_str} | {vol_str}{vol_mark} | {status} |\n"

    # 持仓追踪
    if track['start_date']:
        msg += f"\n---\n📈 **模拟持仓追踪**\n"
        msg += f"- 起始日期：{track['start_date']}\n"
        msg += f"- 累计收益：{(track['simulated_nav']-1)*100:.2f}%\n"
        msg += f"- 累计交易成本：{track['total_cost']*100:.2f}%\n"
        msg += f"- 现金比例：{track['cash']*100:.0f}%\n"

    msg += "\n💡 *进阶稳健版：择时+波动率过滤+交易成本+双持仓+止损*"

    return msg

if __name__ == '__main__':
    # 获取择时信号
    timing = get_market_timing()
    
    # 获取ETF数据
    result = get_latest_signal()
    if result:
        track = load_track()

        # 决策持仓
        current_holdings = track.get('holdings', [])
        new_holdings, changes, total_cost = decide_holdings_advanced(
            result['scores'],
            current_holdings,
            result['prices'],
            result['volatilities'],
            timing
        )

        # 更新追踪
        today = result['date']

        if track['start_date'] is None:
            track['start_date'] = today
            track['holdings'] = new_holdings
            track['total_cost'] = total_cost
            track['timing_state'] = timing[0]
            track['signals'].append({
                'date': today,
                'holdings': [{'code': h['code'], 'name': code_names.get(h['code'], h['code'])} for h in new_holdings],
                'changes': changes,
                'timing': timing[1]
            })
        else:
            # 计算今日收益
            daily_return = 0
            for h in track['holdings']:
                if h['code'] in result['returns']:
                    pos = h.get('position', POSITION_RATIO)
                    daily_return += result['returns'][h['code']] / 100 * pos

            track['simulated_nav'] *= (1 + daily_return)
            track['total_cost'] += total_cost
            track['daily_returns'].append({
                'date': today,
                'return': daily_return
            })

            # 更新持仓
            track['holdings'] = new_holdings
            track['timing_state'] = timing[0]
            if changes:
                track['signals'].append({
                    'date': today,
                    'holdings': [{'code': h['code'], 'name': code_names.get(h['code'], h['code'])} for h in new_holdings],
                    'changes': changes,
                    'timing': timing[1]
                })

        save_track(track)
        print(format_message(result, track, changes, timing))
